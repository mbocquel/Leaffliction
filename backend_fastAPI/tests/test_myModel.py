...
class TestMyCNNModel(unittest.TestCase):
    ...
    def test_load_data(self):
        ...
        if self.model.train_dataset is not None:
    ...
    def test_build(self):
        ...
        if self.model.model is not None:
            ...
            self.assertEqual(
                self.model.model.layers[-2].units, 64
            )  # 64 units for the second last layer
            self.assertEqual(
                self.model.model.layers[-1].units, self.config.model.output_channels
            )  #  self.config.model.output_channels units for the last layer
    ...
    def test_train(self):
        self.model.load_data()
        self.model.build()
        train_loss, val_loss = self.model.train()  # type: ignore
    ...
    def test_predict_dir(self):
        ...
        self.assertTrue(
            np.array([type(predict) == str for predict in predictions]).all()
        )
