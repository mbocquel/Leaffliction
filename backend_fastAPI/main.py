...
def main(**kwargs) -> int:
    try:
        ...
        if kwargs.get("predict"):
            ...
            predict(
                model,
                config.predict.img_path,
                show_result=config.predict.show_result_plot,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train", "-t", action="store_true", help="Launch Train part of the program"
    )
    parser.add_argument(
        "--predict",
        "-p",
    ...
    parser.add_argument(
        "--load", "-l", action="store_true", help="Load an existing model"
    )
