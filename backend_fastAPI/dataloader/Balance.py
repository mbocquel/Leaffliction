...
class Balance:
    ...
    def __init__(self, dir: str, buffer: int = 1) -> None:
        ...
        Args:
            ...
            buffer (int, optional): The buffer value used to determine if the dataset is balanced. Defaults to 1.
        """
        self.imageDir = dir
    ...
    def check_if_balanced(self) -> None:
        ...
        if min(file_count.values()) == 0:
            self.isBalanced = False
            self.balance_status = "Empty category, impossible to balance"
            return
    ...
    def balance_data(self) -> None:
        ...
        categories_to_balance = [
            ...
            if key not in max_categories and val > 0
        ]
    ...
    def balance_category(self, cat: str, max_file: int) -> None:
        ...
        Args:
            cat (str): The category to balance.
            max_file (int): The maximum number of images to have in the category.
        """
    ...
    def get_combinaison_transfo(
        self, files: List[str]
    ) -> Tuple[str, str, Optional[float]]:
        """
        Get a combination of transformations to apply to an image

        Args:
            files (List[str]): The list of files in a category.

        Returns:
            Tuple[str, str, Optional[float]]: A tuple containing the file path, transformation type, and transformation parameter.
        """
        possible_transfo = [
            "rotated",
            "flipped",
        ...
        if selection[1] == "illuminated":
            selection[2] = round(
                0.5 + random.random() * 2, 2
            )  # illum factor from 0.5 to 2.5
    ...
    def remove_augmented_img(self):
        """
        Remove augmented images from the dataset
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", "-p", type=str, help="Path to the directory to balance or clean"
    )
    parser.add_argument(
        "--balance", "-b", action="store_true", help="Balance the directory"
    )
    parser.add_argument(
        "--clean",
        "-c",
